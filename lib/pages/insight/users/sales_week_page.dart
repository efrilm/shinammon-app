...
class SalesWeekPage extends StatefulWidget {
  final String userId;
  SalesWeekPage(this.userId);
  @override
  _SalesWeekPageState createState() => _SalesWeekPageState();
}

class _SalesWeekPageState extends State<SalesWeekPage> {
  var isLoading = false;
  var isData = false;
  ...
  Future<void> getWeekly() async {
    ...
    list.clear();
    final response = await http.get(Uri.parse(AppUrl.getWeekSales +
        "?id=" +
        widget.userId +
        '&project_code=' +
        AppCode.project));
